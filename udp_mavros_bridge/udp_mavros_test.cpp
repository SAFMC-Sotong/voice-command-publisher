...
public:
  ...
  : Node("mavros_example"),
    ...
    yaw_speed_(M_PI / 180.0 * 3.0),  // ~3 degrees per update (rad)
  ...
  {
    // Declare and get the UDP port as a parameter. Default is 14551.
    ...
    RCLCPP_INFO(this->get_logger(), "\033[33mYaw speed (deg/s):\033[0m %f", (yaw_speed_ * 180 / M_PI) / yaw_timer_interval_ms * 1000);
    RCLCPP_INFO(this->get_logger(), "\033[33mAltitude speed (m/s):\033[0m %f", alt_speed_/alt_timer_interval_ms * 1000);
    RCLCPP_INFO(this->get_logger(), "\033[33mPosition speed (m/s):\033[0m %f", pos_speed_/pos_timer_interval_ms * 1000);
    ...
    position_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      ...
      std::bind(&OffboardNode::position_cb, this, std::placeholders::_1));
...
private:
  ...
  void position_cb(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    // Update current position from actual drone position
    // This allows us to adjust our control based on actual position, not just what we think it is
    current_x_ = msg->pose.position.x;
    current_y_ = msg->pose.position.y;
    current_setpoint_z_ = msg->pose.position.z;
    ...
    RCLCPP_DEBUG(this->get_logger(), "Current Position: x=%.2f, y=%.2f, z=%.2f, yaw=%.2f", 
  ...
  void yaw_adjustment_cb() {
    ...
    // Print the current yaw and target yaw occasionally
    RCLCPP_DEBUG(this->get_logger(), "Current Yaw: %.2f rad, Target Yaw: %.2f rad", current_yaw_, target_yaw_);
  }
  ...
  void altitude_adjustment_cb() {
    ...
    // Print the current altitude and target altitude occasionally
    RCLCPP_DEBUG(this->get_logger(), "Current Altitude: %.2f m, Target Altitude: %.2f m", current_setpoint_z_, target_setpoint_z_);
  }
  ...
  void position_adjustment_cb() {
    ...
    // Print the current position and target position occasionally
    RCLCPP_DEBUG(this->get_logger(), "Current Position: (%.2f, %.2f), Target Position: (%.2f, %.2f)", 
                current_x_, current_y_, target_x_, target_y_);
  }
    ...
    // Process any new UDP command (thread-safe)
    {
      ...
      if (new_udp_command_) {
        ...
        // For command 178 (ALTITUDE)
            ...
            // float altitude_change = last_udp_cmd_.param1;
  ...
  // For command 179 (YAW)
    ...
    const double yaw_angle = last_udp_cmd_.param2 ;
    if (last_udp_cmd_.param1 == -1.0f) {
      ...
      RCLCPP_INFO(this->get_logger(), "UDP cmd: Turn right by %.1f°, target: %.2f rad", 
        ...
        } else if (last_udp_cmd_.command == 176) {
          // CUSTOM MODE command
          if (last_udp_cmd_.param1 == 1.0f && last_udp_cmd_.param2 == 6.0f) {
            // Directly set offboard mode
            ...
            if (set_mode_client_->wait_for_service(1s)) {
              ...
              RCLCPP_INFO(this->get_logger(), "Direct OFFBOARD mode command sent");
            } else {
    ...
    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000, 
                     "Publishing setpoint: x=%.2f, y=%.2f, z=%.2f, yaw=%.2f", 
                     current_x_, current_y_, current_setpoint_z_, current_yaw_);
    ...
    if (start_offboard_ && current_state_.mode != "OFFBOARD" &&
        (now - last_request_ > rclcpp::Duration(2s))) {
      auto mode_request = std::make_shared<mavros_msgs::srv::SetMode::Request>();
      mode_request->custom_mode = "OFFBOARD";
      if (set_mode_client_->wait_for_service(1s)) {
        set_mode_client_->async_send_request(mode_request);
    ...
    if (land_and_disarm_ && (now - last_request_ > rclcpp::Duration(2s))) {
      auto mode_request = std::make_shared<mavros_msgs::srv::SetMode::Request>();
      ...
      // Disarm after switching to landing mode
  ...
  void udp_receiver_thread() {
    ...
    // Set socket timeout to make thread joinable on shutdown
    struct timeval tv;
    tv.tv_sec = 1;
    tv.tv_usec = 0;
    ...
    RCLCPP_INFO(this->get_logger(), "UDP receiver thread started on port %d", udp_port_);
    ...
    while (rclcpp::ok() && running_) {
      ...
      if (n == sizeof(UdpCommand)) {
        ...
        RCLCPP_INFO(this->get_logger(), "Received command %d from %s:%d with params: %.2f, %.2f", 
                   cmd.command, sender_ip, ntohs(sender_addr.sin_port), cmd.param1, cmd.param2);
        
        {
          std::lock_guard<std::mutex> lock(udp_mutex_);
          last_udp_cmd_ = cmd;
          last_udp_cmd_.param1 = cmd.param1;
          new_udp_command_ = true;
        }
      } else {
        RCLCPP_WARN(this->get_logger(), "Unexpected UDP packet size: %ld bytes (expected %ld)", 
                    n, sizeof(UdpCommand));
      }
    }
    
    close(sock_fd);
    RCLCPP_INFO(this->get_logger(), "UDP receiver thread shutdown");
  }
  ...
  // Target variables for gradual adjustment.
